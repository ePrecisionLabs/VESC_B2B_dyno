...
def run_b2b_dyno_test(config):
    ...
    number_loops = int(config['number_loops'])
    ...
    with open(log_file_name_ext, "w") as log_file:
        with VESC(serial_port=motor_dut_port, timeout=0.1) as motor_dut:
            with VESC(serial_port=motor_absorber_port, timeout=0.1) as motor_absorber:
                ...
                for i in range(number_loops):
                    log_file.write(f"Iteration {i} out of {number_loops}\n")
                    for idx, rpm in enumerate(rpm_array):
                    ...
                    log_file.write(f'End of test sequence iteration {i}\n')
